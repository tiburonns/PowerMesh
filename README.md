...
> **Current `main`: 0.2.0 (build 14).** Feature-complete for the first signed ecosystem acceptance pass. The remaining release gate is physical-device validation of Apple-provisioned capabilities and real hardware behavior.
...
> **`main` actual: 0.2.0 (build 14).** Funcionalmente completo para la primera aceptación firmada del ecosistema. La puerta restante es validar en hardware real las capacidades aprovisionadas por Apple.
